...
fn vorticity(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let dx = abs(textureLoad(curlTexture, vec2<i32>(i, j-1)).x) - abs(textureLoad(curlTexture, vec2<i32>(i, j+1)).x);
    let dy = abs(textureLoad(curlTexture, vec2<i32>(i+1, j)).x) - abs(textureLoad(curlTexture, vec2<i32>(i-1, j)).x);
